...
--let's explain each line of the SQL code in detail:
...
--go
--This line is not a part of the SQL query itself. It is often used as a delimiter to separate batches of SQL statements. It indicates the end of the current batch.
...
AS
BEGIN
  IF EXISTS(SELECT 1 FROM inserted i JOIN Vehicle v ON i.VehicleRegNo = v.VehicleRegNo WHERE v.Status <> 'available')
  BEGIN
  ...
  ELSE
  BEGIN
    INSERT INTO Route (VehicleRegNo, DriverID, DepartureTime, ArrivalTime, Distance, FuelConsumed, AverageSpeed)
    SELECT VehicleRegNo, DriverID, DepartureTime, ArrivalTime, Distance, FuelConsumed, AverageSpeed
    FROM inserted;
  END;
END;
...
---show ms sql server db backup with gui and also with code
...
---show ms sql server sql query code db Commit
